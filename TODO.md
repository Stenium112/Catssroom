- [ ] reworj the window system with one single window ! Using mouse passtrough only ***around the mouse !!!!!!!!!***

- [ ] make a update checker with this : https://docs.godotengine.org/en/stable/tutorials/networking/http_request_class.html

- [ ] finish the bar and chrono

- [ ] publish the first release

- [ ] make good logo for button and app

- [ ] make traduction for English/French

- [ ] make comment on the code
